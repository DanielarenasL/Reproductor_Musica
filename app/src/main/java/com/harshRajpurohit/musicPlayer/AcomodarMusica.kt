...
import com.harshRajpurohit.musicPlayer.AcomodarMusica.MyHolder
...
class AcomodarMusica(private val context: Context, private var musicList: ArrayList<Musica>, private val playlistDetails: Boolean = false,
        ...
        if(!selectionActivity)
            holder.root.setOnLongClickListener {
                ...
                bindingMF.AddToPNBtn.setOnClickListener {
                    try {
                        if(ReproduccirSiguiente.playNextList.isEmpty()){
                            ReproduccirSiguiente.playNextList.add(ActividadUsuario.musicListPA[ActividadUsuario.songPosition])
                            ActividadUsuario.songPosition = 0
                        }

                        ReproduccirSiguiente.playNextList.add(musicList[position])
                        ActividadUsuario.musicListPA = ArrayList()
                        ActividadUsuario.musicListPA.addAll(ReproduccirSiguiente.playNextList)
                    }catch (e: Exception){
...
//                        .setBackground(ColorDrawable(0x99000000.toInt()))
        ...
        when{
            ...
            selectionActivity ->{
                ...
                holder.root.setOnClickListener {
                when{
                    MainActivity.search -> sendIntent(ref = "AdaptadorDeMusica", pos = position)
                    musicList[position].id == ActividadUsuario.nowPlayingId ->
                        sendIntent(ref = "Reproduciendo", pos = ActividadUsuario.songPosition)
                    else->sendIntent(ref="AcomodarMusica", pos = position) } }
        }
    ...
    fun updateMusicList(searchList : ArrayList<Musica>){
        musicList = ArrayList()
    ...
    private fun sendIntent(ref: String, pos: Int){
        ...
        intent.putExtra("Clase", ref)
        ContextCompat.startActivity(context, intent, null)
    }
    private fun addSong(song: Musica): Boolean{
        ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist.forEachIndexed { index, musica ->
            if(song.id == musica.id){
                ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist.removeAt(index)
                return false
            }
        }
        ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist.add(song)
        return true
    }
    fun refreshPlaylist(){
        musicList = ArrayList()
        musicList = ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist
        notifyDataSetChanged()
    }
}