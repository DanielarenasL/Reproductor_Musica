...
class ActividadUsuario : AppCompatActivity(), ServiceConnection, MediaPlayer.OnCompletionListener {


    companion object {
        lateinit var musicListPA : ArrayList<Musica>
        var songPosition: Int = 0
        var isPlaying:Boolean = false
        var musicService: ServicioMusica? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.data?.scheme.contentEquals("content")){
            songPosition = 0
            val intentService = Intent(this, ServicioMusica::class.java)
        ...
        binding.favouriteBtnPA.setOnClickListener {
            ...
            if(isFavourite){
                ...
                FuncionFavoritos.favouriteSongs.removeAt(fIndex)
            } else{
                isFavourite = true
                binding.favouriteBtnPA.setImageResource(R.drawable.favourite_icon)
                FuncionFavoritos.favouriteSongs.add(musicListPA[songPosition])
            }
            FuncionFavoritos.favouritesChanged = true
        }
    }
//Important Function
    private fun initializeLayout(){
        ...
        when(intent.getStringExtra("class")){
            ...
            "FavouriteAdapter"-> initServiceAndPlaylist(FuncionFavoritos.favouriteSongs, shuffle = false)
            "MainActivity"-> initServiceAndPlaylist(MainActivity.MusicListMA, shuffle = true)
            "FavouriteShuffle"-> initServiceAndPlaylist(FuncionFavoritos.favouriteSongs, shuffle = true)
            "PlaylistDetailsAdapter"->
                initServiceAndPlaylist(ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist, shuffle = false)
            "PlaylistDetailsShuffle"->
                initServiceAndPlaylist(ListaReproduccion.musicPlaylist.ref[DetallesListaReproduccion.currentPlaylistPos].playlist, shuffle = true)
            "PlayNext"->initServiceAndPlaylist(ReproduccirSiguiente.playNextList, shuffle = false, playNext = true)
        }
    ...
    override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
        if(musicService == null){
            val binder = service as ServicioMusica.MyBinder
    ...
    override fun onCompletion(mp: MediaPlayer?) {
        ...
        Reproduciendo.binding.songNameNP.isSelected = true
        Glide.with(applicationContext)
            .load(musicListPA[songPosition].artUri)
            .apply(RequestOptions().placeholder(R.drawable.music_player_icon_slash_screen).centerCrop())
            .into(Reproduciendo.binding.songImgNP)
        Reproduciendo.binding.songNameNP.text = musicListPA[songPosition].title
    }
    ...
    private fun showBottomSheetDialog(){
        val dialog = BottomSheetDialog(this@ActividadUsuario)
    ...
    @RequiresApi(Build.VERSION_CODES.Q)
    private fun getMusicDetails(contentUri: Uri): Musica{
        ...
        try {
            ...
            return Musica(id = "Unknown", title = path.toString(), album = "Unknown", artist = "Unknown", duration = duration,
    ...
    private fun initServiceAndPlaylist(playlist: ArrayList<Musica>, shuffle: Boolean, playNext: Boolean = false){
        ...
        setLayout()
        if(!playNext) ReproduccirSiguiente.playNextList = ArrayList()
    }
}